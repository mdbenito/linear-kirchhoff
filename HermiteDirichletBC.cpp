...
const std::set<std::string> HermiteDirichletBC::methods
= {"topological", "geometric", "pointwise"};

//-----------------------------------------------------------------------------
HermiteDirichletBC::HermiteDirichletBC(std::shared_ptr<const FunctionSpace> V,
                         std::shared_ptr<const GenericFunction> g,
                         std::shared_ptr<const SubDomain> sub_domain,
                         std::string method,
                         bool check_midpoint)
  : Hierarchical<HermiteDirichletBC>(*this), _function_space(V), _g(g),
...
//-----------------------------------------------------------------------------
HermiteDirichletBC::HermiteDirichletBC(const HermiteDirichletBC& bc)
  : Hierarchical<HermiteDirichletBC>(*this)
{
  *this = bc;
}
//-----------------------------------------------------------------------------
HermiteDirichletBC::~HermiteDirichletBC()
{
  // Do nothing
}
//-----------------------------------------------------------------------------
const HermiteDirichletBC& HermiteDirichletBC::operator= (const HermiteDirichletBC& bc)
{
  ...
  Hierarchical<HermiteDirichletBC>::operator=(bc);

  return *this;
}
//-----------------------------------------------------------------------------
void HermiteDirichletBC::apply(GenericMatrix& A) const
{
  apply(&A, 0, 0);
}
//-----------------------------------------------------------------------------
void HermiteDirichletBC::apply(GenericVector& b) const
{
  apply(0, &b, 0);
}
//-----------------------------------------------------------------------------
void HermiteDirichletBC::apply(GenericMatrix& A, GenericVector& b) const
{
  apply(&A, &b, 0);
}
//-----------------------------------------------------------------------------
void HermiteDirichletBC::apply(GenericVector& b, const GenericVector& x) const
{
  apply(0, &b, &x);
}
//-----------------------------------------------------------------------------
void HermiteDirichletBC::apply(GenericMatrix& A,
                        GenericVector& b,
...
//-----------------------------------------------------------------------------
void HermiteDirichletBC::gather(Map& boundary_values) const
{
  Timer timer("HermiteDirichletBC gather");
...
//-----------------------------------------------------------------------------
void HermiteDirichletBC::get_boundary_values(Map& boundary_values) const
{
  // Create local data
...
//-----------------------------------------------------------------------------
void HermiteDirichletBC::zero(GenericMatrix& A) const
{
  // Check arguments
...
//-----------------------------------------------------------------------------
void HermiteDirichletBC::zero_columns(GenericMatrix& A,
                               GenericVector& b,
...
//-----------------------------------------------------------------------------
const std::vector<std::size_t>& HermiteDirichletBC::markers() const
{
  return _facets;
}
//-----------------------------------------------------------------------------
std::shared_ptr<const GenericFunction> HermiteDirichletBC::value() const
{
  return _g;
}
//-----------------------------------------------------------------------------
std::shared_ptr<const SubDomain> HermiteDirichletBC::user_sub_domain() const
{
  return _user_sub_domain;
}
//-----------------------------------------------------------------------------
void HermiteDirichletBC::homogenize()
{
...
//-----------------------------------------------------------------------------
void HermiteDirichletBC::set_value(std::shared_ptr<const GenericFunction> g)
{
  _g = g;
}
//-----------------------------------------------------------------------------
std::string HermiteDirichletBC::method() const
{
  return _method;
}
//-----------------------------------------------------------------------------
void HermiteDirichletBC::apply(GenericMatrix* A,
                        ...
                        const GenericVector* x) const
{
  Timer timer("HermiteDirichletBC apply");
...
//-----------------------------------------------------------------------------
void HermiteDirichletBC::check() const
{
  dolfin_assert(_g);
...
//-----------------------------------------------------------------------------
void HermiteDirichletBC::init_facets(const MPI_Comm mpi_comm) const
{
  Timer timer("HermiteDirichletBC init facets");
...
//-----------------------------------------------------------------------------
void HermiteDirichletBC::init_from_mesh(std::size_t sub_domain) const
{
...
//-----------------------------------------------------------------------------
void HermiteDirichletBC::compute_bc(Map& boundary_values, LocalData& data,
                             std::string method) const
{
  Timer timer("HermiteDirichletBC compute bc");
...
//-----------------------------------------------------------------------------
void HermiteDirichletBC::compute_bc_topological(Map& boundary_values,
                                         LocalData& data) const
{
  ...
  boundary_values.reserve(boundary_values.size()
                          + _facets.size()*(dofmap.num_facet_dofs()-2));  // HACK
...
//-----------------------------------------------------------------------------
void HermiteDirichletBC::compute_bc_geometric(Map& boundary_values,
                                       LocalData& data) const
{
  dolfin_assert(_function_space);
  dolfin_assert(_function_space->element());
...
//-----------------------------------------------------------------------------
void HermiteDirichletBC::compute_bc_pointwise(Map& boundary_values,
                                       LocalData& data) const
{
  if (!_user_sub_domain)
  {
...
//-----------------------------------------------------------------------------
bool HermiteDirichletBC::on_facet(const double* coordinates, const Facet& facet) const
{
...
//-----------------------------------------------------------------------------
void HermiteDirichletBC::check_arguments(GenericMatrix* A, GenericVector* b,
...
//-----------------------------------------------------------------------------
HermiteDirichletBC::LocalData::LocalData(const FunctionSpace& V)
